...
RSpec.describe ProductCollection do
  before do
    ...
    (0..3).each do |n|
      @collection << Product.new(n, Faker::GameOfThrones.character, Faker::Commerce.price)
    end
  end
  ...
  describe "#validate_product_code_is_uniq(code)" do
    context "when a product with a similar code exists" do
      it "raises DuplicateProductCodeError" do
        expect { @collection.validate_product_code_is_uniq(0) }.to raise_error("DuplicateProductCodeError")
      end
    end
